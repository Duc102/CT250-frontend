...
const ProductItemLine = (props) => {
    ...
    function changeQty(number) {
        ...
        if (props.productLine.qty <= 0)
            props.productLine.qty = 1;
        if (props.productLine.qty > props.productLine.productItem.qtyInStock)
            props.productLine.qty = props.productLine.productItem.qtyInStock
        else {
            setUpdate(!update);
            let ss = shoppingCart.shoppingCartItems.filter(it => it.id.productItemId === props.productLine.productItem.id);
            ss[0].qty = props.productLine.qty;
